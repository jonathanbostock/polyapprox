...
from polyapprox.relu import relu, relu_prime, relu_ev, relu_prime_ev, relu_squared_ev
...
@given(st.floats(-4, 4), st.floats(0.1, 10))
def test_relu_squared_evs(mu, sigma):
    analytic_ev = relu_squared_ev(mu, sigma)
    numerical_ev, err = quad(
        lambda x: relu(x) ** 2 * norm.pdf(x, loc=mu, scale=sigma), -np.inf, np.inf,
    )
    np.testing.assert_allclose(numerical_ev, analytic_ev, atol=err)